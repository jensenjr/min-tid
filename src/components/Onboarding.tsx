...
import {
  ...
  DEFAULT_SCHEDULE,
  shiftMinutes, weeklyNetMin, fmtMin,
} from "../lib/schedule";
...
type Step = "name" | "schedule" | "dept";

// ─── Time picker — hour:minute selects snapping to 5-min intervals ────────────
function TimePicker({
  value,
  onChange,
  disabled,
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "1px" }}>
      <select value={h} onChange={e => setH(+e.target.value)} disabled={disabled} style={base}>
      ...
      <span style={{
        ...
        color: disabled ? "#c4b0a0" : "#2d1717",
        userSelect: "none", padding: "0 1px",
      }}>:</span>
      <select value={m} onChange={e => setM(+e.target.value)} disabled={disabled} style={base}>
...
// ─── Weekly schedule editor — shared between Onboarding and SettingsModal ────
export function WeekScheduleEditor({
  ...
  function applyBulk() {
    ...
    for (const k of DAY_KEYS) {
      if (next[k].active) {
        next[k] = { ...next[k], startTime: bulkStart, endTime: bulkEnd };
      }
    }
    onChange(next);
  }
  ...
  return (
    <div className="w-full space-y-2">
      ...
      {DAY_KEYS.map(key => {
        ...
        const mins = cfg.active ? shiftMinutes(cfg) : 0;

        return (
          <div
            key={key}
            className="flex items-center gap-2 bg-white rounded-[16px] px-3 py-2.5 border border-pc-line transition-opacity"
            style={{ opacity: cfg.active ? 1 : 0.45 }}
          >
            {/* Day label */}
            <span
              className="text-[13px] font-extrabold shrink-0"
              style={{ width: 30, color: isWeekend ? "#c4a882" : "#2d1717" }}
            >
              {DAY_LABEL_SHORT[key]}
            </span>
            ...
            <span
              ...
              style={{ width: 36, textAlign: "right", color: cfg.active ? "#9a6a3a" : "#c4b0a0" }}
            >
              {cfg.active ? fmtMin(mins) : "—"}
            </span>
      ...
      <div className="mt-1 bg-pc-peach rounded-2xl px-4 py-3 flex items-center justify-between">
        <span className="text-[12px] font-bold text-pc-orange-deep uppercase tracking-wide">Totalt / vecka</span>
...
// Keep the old export name so SettingsModal doesn't break until updated
export { WeekScheduleEditor as PerDayEditor };
...
export default function Onboarding({ onComplete }: { onComplete: (r: OnboardingResult) => void }) {
      ...
      <div className="w-full space-y-4">
        <div>
          ...
          <input
            ...
            onKeyDown={e => { if (e.key === "Enter" && name.trim()) setStep("schedule"); }}
            className="ob-input"
          />
  ...
  if (step === "schedule") return (
    <Screen scroll>
      ...
      <p className="text-[13px] text-pc-muted mb-5 text-center leading-relaxed">
        Ange när du börjar och slutar varje dag. Du kan ändra detta när som helst.
      </p>

      <WeekScheduleEditor schedule={schedule} onChange={setSchedule} />

      <div className="grid grid-cols-2 gap-3 pt-4 w-full">
        <button onClick={() => setStep("name")} className="ob-btn-ghost">← Tillbaka</button>
        <button onClick={() => setStep("dept")} className="ob-btn-primary">Nästa →</button>
      </div>
    </Screen>
  );
